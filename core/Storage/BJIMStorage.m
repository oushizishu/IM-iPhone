...
- (long)sumOfAllConversationUnReadNumOwnerId:(int64_t)ownerId userRole:(IMUserRole)userRole
{
    ...
    [self.dbHelper executeDB:^(FMDatabase *db) {
        NSString *query = [NSString stringWithFormat:@"select sum(unReadNum) from CONVERSATION where ownerId=%lld and ownerRole=%ld and relation=%ld and toId<>%ld and toId<>%ld", ownerId, (long)userRole, (long)eConverastion_Relation_Normal, (long)USER_FRESH_FANS, (long)USER_STRANGER];
